...
namespace HustlerNation
{
    ...
    options => options.UseMySql(Configuration.GetConnectionString("hustler")));
            //        services.AddDbContext<ApplicationDBContext_online>(
            //options => options.UseMySql(Configuration.GetConnectionString("online")));
            services.AddTransient<MySqlConnection>(_ => new MySqlConnection(Configuration["hustler"]));
